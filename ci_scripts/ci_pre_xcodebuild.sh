...
if [ -z "${DEVELOPMENT_TEAM:-}" ]; then
	...
	echo "ci_pre_xcodebuild: Xcode Cloud passes CODE_SIGN_IDENTITY=- on the archive command line, so the archive stays ad-hoc. With Notarize configured, ci_post uses CI_DEVELOPER_ID_SIGNED_APP_PATH. See docs/release.md."
	exit 0
fi
...
# written here. ci_post uses CI_DEVELOPER_ID_SIGNED_APP_PATH when Notarize
# is configured, and codesigns only when that variable is unset.
...
// With Notarize configured, ci_post uses CI_DEVELOPER_ID_SIGNED_APP_PATH.
...
echo "ci_pre_xcodebuild: Xcode Cloud passes CODE_SIGN_IDENTITY=- on the archive command line, which overrides this overlay. The archive is ad-hoc (Sign to Run Locally). With Notarize configured, ci_post uses CI_DEVELOPER_ID_SIGNED_APP_PATH. See docs/release.md."
