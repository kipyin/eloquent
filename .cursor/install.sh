...
# Linux Cloud VMs cannot build or run this Xcode app. Per-boot identity
# re-apply lives in start.sh.
set -euo pipefail

CURSOR_DIR="$(cd "$(dirname "$0")" && pwd)"
cd "$CURSOR_DIR/.."

# Official Matt 25 + global extras, plus eloquent/ project skills from kipyin/kstack.
...
bash /tmp/kstack/install.sh eloquent

# House git identity (author, signing, commit-msg strip). Durable under
# ~/.cursor/kstack-hooks; start.sh re-applies from the clone each session.
